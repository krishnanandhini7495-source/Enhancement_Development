...
const LOCAL_IP = '192.168.99.10'; // Your PC's actual IP address
...
const getBaseURL = (): string => {
  if (__DEV__) {
    ...
    if (Platform.OS === 'android') {
      ...
      // For physical device, uncomment the line below and comment the emulator line
      return 'http://10.0.2.2:5000/api'; // For emulator
      // return `http://${LOCAL_IP}:5000/api`; // For physical device
    } else {
